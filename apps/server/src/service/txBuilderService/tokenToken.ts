...
const SWAP_REQ_CAPACITY = 227n * constants.CKB_DECIMAL;
...
export class TokenTokenTxBuilderService implements TxBuilderService {
  ...
  public async buildCreateLiquidityPool(
    ...
    const cellDeps = [tokenTokenConfig.INFO_TYPE_DEP, config.SUDT_TYPE_DEP]
      .concat(userLockDeps)
      .concat([config.INFO_LOCK_DEP, config.INFO_LOCK_DEP, config.INFO_LOCK_DEP]);
    const witnessArgs =
