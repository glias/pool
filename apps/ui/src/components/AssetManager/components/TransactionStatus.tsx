...
import { TransactionStatus } from '@gliaswap/commons';
...
// @ts-ignore
const PendingIcon: typeof Icon = (props) => <Icon {...props} component={PendingSvg} />;
...
const TransactionStatusIconWrapper = styled.div<{
  ...
  ${({ status, filled, direction }) => {
    let color: string;
    if (status !== 'committed') color = '#FF9C72';
    else if (direction === 'in') color = '#72D1A4';
    else color = '#57C2FF';
...
export const TransactionStatusIcon: React.FC<TransactionDirectionStatusProps> = (
  ...
  if (status !== 'committed') IconComponent = PendingIcon;
  else if (direction === 'in') IconComponent = ArrowDownOutlined;
  else if (direction === 'out') IconComponent = ArrowUpOutlined;
  ...
  return (
    <TransactionStatusIconWrapper
      ...
      height={height || width}
    >
      <IconComponent style={{ fontSize: `${(width || height) - 6}px` }} />
    </TransactionStatusIconWrapper>
  );
};
